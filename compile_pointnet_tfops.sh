CUDA_INCLUDE=' -I/usr/local/cuda-12.4/include/'
CUDA_LIB=' -L/usr/local/cuda-12.4/lib64/'
TF_CFLAGS=$(python3 -c 'import tensorflow as tf; print(" ".join(tf.sysconfig.get_compile_flags()))')
TF_LFLAGS=$(python3 -c 'import tensorflow as tf; print(" ".join(tf.sysconfig.get_link_flags()))')
cd pointnet2/tf_ops/sampling

/usr/local/cuda-12.4/bin/nvcc -std=c++11 -c -o tf_sampling_g.cu.o tf_sampling_g.cu \
...
/usr/local/cuda-12.4/bin/nvcc -std=c++11 -c -o tf_grouping_g.cu.o tf_grouping_g.cu \
 ${CUDA_INCLUDE} ${TF_CFLAGS} -D GOOGLE_CUDA=1 -x cu -Xcompiler -fPIC
